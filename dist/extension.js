...
function registerCommand2(context) {
  const disposable = vscode2.commands.registerCommand("dev-pocket.envJsonConvert", async () => {
    ...
    if (!selection.isEmpty) {
      ...
      const fullRange = new vscode2.Range(0, 0, document.lineCount - 1, document.lineAt(document.lineCount - 1).text.length);
      content = document.getText(fullRange);
    }
    if (!content.trim()) {
      vscode2.window.showErrorMessage("No content to convert.");
      return;
    }
    try {
      ...
      vscode2.window.showErrorMessage(`Conversion failed: ${message}`);
    }
  });
  context.subscriptions.push(disposable);
}
...
function registerCommand3(context) {
  const disposable = vscode3.commands.registerCommand("dev-pocket.base64Encode", async () => {
    ...
    if (!content.trim()) {
      vscode3.window.showErrorMessage("No content to encode.");
      return;
    }
    try {
      ...
      const message = error instanceof Error ? error.message : "Unknown error occurred.";
      vscode3.window.showErrorMessage(`Encoding failed: ${message}`);
    }
  });
  context.subscriptions.push(disposable);
}
...
function registerCommand4(context) {
  const disposable = vscode4.commands.registerCommand("dev-pocket.base64Decode", async () => {
    ...
    if (!content.trim()) {
      vscode4.window.showErrorMessage("No content to decode.");
      return;
    }
    try {
      ...
      const message = error instanceof Error ? error.message : "Unknown error occurred.";
      vscode4.window.showErrorMessage(`Decoding failed: ${message}`);
    }
  });
  context.subscriptions.push(disposable);
}
...
function registerCommand5(context) {
  const disposable = vscode5.commands.registerCommand("dev-pocket.jsonMinify", async () => {
    ...
    if (!content.trim()) {
      vscode5.window.showErrorMessage("No content to minify.");
      return;
    }
    try {
      await vscode5.languages.setTextDocumentLanguage(document, "json");
      const trimmed = content.trim();
      ...
      const message = error instanceof Error ? error.message : "Unknown error occurred.";
      vscode5.window.showErrorMessage(`Minification failed: ${message}`);
    }
  });
  context.subscriptions.push(disposable);
}
...
function registerCommand6(context) {
  const disposable = vscode6.commands.registerCommand("dev-pocket.unixConvert", async () => {
    ...
    if (!content.trim()) {
      vscode6.window.showErrorMessage("No content to convert.");
      return;
    }
    const converted = findAllTimestamps(content);
    if (converted.length === 0) {
      vscode6.window.showErrorMessage("No valid unix timestamp found.");
      return;
    }
...
${c.formatted}`).join("\n\n");
    ...
    await vscode6.window.showTextDocument(doc, { viewColumn: vscode6.ViewColumn.Beside });
  });
  context.subscriptions.push(disposable);
}
...
var UnixHoverProvider = class {
  provideHover(document, position, token) {
    ...
    return new vscode7.Hover(markdown, range);
  }
};
function registerProvider(context) {
  const provider = new UnixHoverProvider();
  const disposable = vscode7.languages.registerHoverProvider(
    { pattern: "**/*" },
    provider
  );
  context.subscriptions.push(disposable);
...
function activate(context) {
  ...
  const config = vscode8.workspace.getConfiguration("dev-pocket");
